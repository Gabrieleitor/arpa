...
class ArticleRecordControllerTest {
    ...
    void registerEntry_success() throws Exception {
        when(handler.registerEntry(any(), any(), any(), any(), any(), any(), any(), any())).thenReturn(response);
    ...
    void registerEntry_error() throws Exception {
        when(handler.registerEntry(any(), any(), any(), any(), any(), any(), any(), any())).thenThrow(new IllegalArgumentException("Identification type not found"));
        mockMvc.perform(post("/api/articles/entry")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
